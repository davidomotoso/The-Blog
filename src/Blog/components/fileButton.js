const FileButton = ({ handleImg, handleVideo }) => {
  return (
    <div className="absolute left-2 bottom-2 z-20 w-[97%]">
      <div className="text-2xl text-purple-600  flex items-center gap-x-3 bg-gray-200 rounded-full py-2 px-3 w-56">
        <button
          type="submit"
        ...
        <button
          type="submit"
          className="w-10 h-10 rounded-full hover:shadow-lg hover:bg-white"
        >
          <i className="bi bi-journals" title="Add a new section"></i>
        </button>
        <button
          type="submit"
          className="w-10 h-10 rounded-full hover:shadow-lg hover:bg-white"
        >
          <i className="bi bi-card-list" title="Add a list"></i>
        </button>
      </div>
    </div>
  );
